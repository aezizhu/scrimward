...
(incl. Claude Code) to **compress context**. It is the closest prior art to Scrimward's
...
➡️ **Scrimward does the same: transparent header reverse-proxy; mutate only the JSON body.**
...
> differs. Scrimward reuses this exact shape; we add a redact/un-mask adapter per provider on top.
...
| Headroom (fail-open) | Scrimward (fail-closed) |
|---|---|
...
**Launcher (`scrimward wrap <tool>`, e.g. `scrimward wrap claude`):** Click CLI; set env **and** `settings.local.json`;
...
prior art. Scrimward is an independent implementation with the opposite (fail-closed) safety model.*
